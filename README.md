...
- **Controls** — Play/Pause, a discrete speed-multiplier slider, Camera mode (`Free` / `Follow selected UAV`), visibility toggles (vertiports, corridors, routes, flight envelopes, buildings, roads, trees, labels), a **Shadows** on/off toggle (building/tree shadows plus drones' altitude-faded ground shadows), a **Reset view** button (animated fly-back to the initial camera framing), and **Reset simulation**.
