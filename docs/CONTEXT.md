...
3. `loadUavModels()` — one model per vehicle type (`quadrotor`/`fixedwing`/`hybrid`.gltf), each preserving its gltf materials and normalized to a per-type span (`DRONE_MODEL_SPAN_METERS_BY_TYPE`); a missing type falls back to a cone.
...
- **Vertiports**: camera-oriented helipad-style ground decals.
