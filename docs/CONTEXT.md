...
Ground icon artwork lives outside `data/`, under `public/icons/` (`vertiport.svg` plus `nav_site.svg`, `comm_site.svg`, `surv_site.svg` awaiting placement). These are always bundled — never supplied by the backend or by uploads — and are listed in `GROUND_ICON_ASSETS`.
...
- `vertiport.ts`, `flows.ts` — vertiport markers and demand flows.

## Scene Data Model

Core shared types are in `src/types.ts`. Telemetry DTOs are in `src/telemetry/protocol.ts`.

- `SceneData` — complete frontend scene payload: `origin`, `sceneBounds`, `corridors`, `routes`, `buildings`, `roads`, `trees`, `vertiports`, `flows`.
