...
- Rendering: Three.js; `manifold-3d` (WASM, initialized during bootstrap) performs all build-time CSG — flight-envelope junction fusion and the CNS coverage shell's map-edge cut.
...
Ground icon artwork lives outside `data/`, under `public/icons/` (`vertiport.svg` for vertiport terminals; `cont_site.svg` for contingency landing sites; `nav_site.svg`, `comm_site.svg`, `surv_site.svg` for CNS sites). These are always bundled — never supplied by the backend or by uploads — and are listed in `GROUND_ICON_ASSETS`.
...
- `vertiport.ts`, `contingencySite.ts`, `cnsSite.ts`, `flows.ts` — vertiport markers, contingency landing sites, CNS sites, and demand flows.

## Scene Data Model

Core shared types are in `src/types.ts`. Telemetry DTOs are in `src/telemetry/protocol.ts`.

- `SceneData` — complete frontend scene payload: `origin`, `sceneBounds`, `corridors`, `routes`, `buildings`, `roads`, `trees`, `vertiports`, `contingencySites`, `sites`, `flows`.
...
- `ContingencySite` — an optional contingency landing site (`node_type` = `cont_site`), a standalone emergency-landing node rendered exactly like a vertiport marker. Never referenced by corridor ways, so it carries no connectivity; its `altitude` tag is ground elevation (the marker stays on the ground plane) and sim-side tags (`max_landing_rate_per_hour`, `vehicle_access`) are ignored by the UI, like a vertiport's `pad_counts`. Its `node_id` is a display label only — the schema reuses values across node types — so nothing may key on it.
...
- **Contingency landing sites** (`src/layer/contingencySite.ts`): optional standalone emergency-landing spots, another client of the shared ground icon layer — the same camera-oriented ground decals as vertiports (`cont_site.svg`, the same footprint), toggled by their own "Contingency Sites" checkbox.
