...
### Step 4. Asset preload (`loadUavModels()` + `loadGroundIconTextures()` + `initCsg()`)
All three run in parallel from `main.ts` behind the loading overlay (concurrent with Step 2's source fetch), and all produce **long-lived masters** that outlive any single `FleetScene`.

**CSG module** (`initCsg()` from `src/geometry/csg.ts`)
- **Input**: the `manifold-3d` WASM binary (~530 kB asset, streamed and compiled by the browser during download).
- **Action**: instantiates and initializes the Manifold module — the boolean-geometry engine Step 6 calls synchronously for envelope junction fusion (`csgUnion`) and the coverage shell's map-edge cut (`csgIntersect`).
- **Output**: a module-level singleton inside `src/geometry/csg.ts`; scene rebuilds reuse it, nothing disposes it. Init is ~20 ms plus the fetch, fully hidden inside the (much larger) Step 2 source fetch.
...
7. **Fuse** (`buildComponentEnvelope`):
   ...
   - **Otherwise →** chain tubes + spheres are converted to `manifold-3d` solids (position-only, vertices welded via `Mesh.merge()`) and **CSG-unioned** in one n-ary `Manifold.union` call into one watertight blob; normals are recomputed on the output. If Manifold rejects a degenerate input, it falls back to a plain (overlapping) `mergeGeometries` so a single bad junction never blanks the whole layer. The WASM module is initialized once during app bootstrap (`initCsg`), concurrent with the scene-source fetches.

   Splitting by miterability is what keeps this cheap: only true junctions and genuinely unmiterable corners pay for CSG, and one tube per maximal chain keeps union inputs ~10× fewer than one solid per edge and one sphere per node would — and simplification typically makes sharp joints miterable again, so splits stay rare. Vertiport nodes never get a sphere and always end a chain, so a vertiport terminal is just a clean flat cap.
