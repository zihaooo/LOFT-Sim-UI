...
### Step 4. UAV model load (`loadUavModels()` from `src/geometry/drone.ts`)
...
| 6h. Vertiports (`createVertiportGroup`) | `VertiportPoint[]` | One flat `CircleGeometry` ground disc per vertiport, all sharing **one** geometry + **one** canvas-texture (`MeshBasicMaterial`, the painted "V" helipad marking). Spun upright per frame (Phase B Step 5) | `renderOrder = VERTIPORT_RENDER_ORDER` (1). Opaque pass; `depthFunc:AlwaysDepth` (ignores buildings) + `depthWrite` + `polygonOffset:-8` so roads/ground stay below it but the airspace layer (order 2) still occludes it |
...
### Step 5. Vertiport billboards (`updateVertiportBillboards`)
- Projects the camera's look direction onto the ground plane and sets every marker's `rotation.y` so its "V" stays upright and readable. The discs stay flat; one shared rotation for all markers. Pure transform update.
...
2. **Opaque pass** — sky dome, ground + grid, buildings, roads, trees (two instanced draws), vertiport markers, the corridor centerline fat-line + direction-cone instanced mesh, and the per-type UAV instanced meshes' opaque parts (each bound to its freshly uploaded `instanceMatrix`/`instanceColor`). Render order keeps the airspace layer above the vertiport markers.
...
- **Static VBO/IBOs** (uploaded once at load): ground, grid, buildings (merged), roads (merged), sky dome, vertiport discs, corridor centerlines (merged fat lines, vertex-colored), envelopes (merged, vertex-colored), tree trunk + canopy geometries, shared cone geometry.
