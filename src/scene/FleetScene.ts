...
export class FleetScene {
  ...
  private addFlightEnvelopes(routes: AirRoute[]): void {
    routes.forEach((route) => {
      const curve = new THREE.CatmullRomCurve3(route.points.map(toVector3), false, "catmullrom", 0);
      const geometry = new THREE.TubeGeometry(curve, Math.max(route.points.length * 12, 32), route.envelopeRadius, 14, false);
      const material = new THREE.MeshStandardMaterial({
        color: route.color,
        transparent: true,
        opacity: 0.16,
